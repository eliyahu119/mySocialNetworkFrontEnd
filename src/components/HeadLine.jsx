...
export default function HeadLine() {
    ...
    return (
        <nav className={`headLine  ${open ? 'open' : ''} `}>
            {' '}
            {/*the navbar*/}
            <i className="z-20">
            ...
            <div className={`linksConteiner ${open ? 'open' : ''}`}>
                <div className="InfoLinksConteiner">
                    {' '}
                    {/*general conteiner */}
                    <NavLink
                        onClick={constCloseOpenWindow}
                        className={'InfoLinks'}
                        to={'/'}
                    >
                        {'Home'}
                ...
                <div className="LoglinksConteiner">
                    {' '}
                    {/*the login and signin div */}
                    {!localStorage.getItem('token') ? (
                        <>
                            <NavLink
                                onClick={constCloseOpenWindow}
                                className="SignInLoginLinks"
                                to={'/Login'}
                            >
                                {'Login'}
                            </NavLink>
